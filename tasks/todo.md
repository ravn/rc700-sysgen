...
- [ ] **Convert ISRs to clang `__attribute__((interrupt))` C functions.**  Future cleanup.  The attribute is fully supported in llvm-z80 (`Z80FrameLowering.cpp:386-398`, `Z80RegisterInfo.cpp:104-112`); without `+shadow-regs` the CSR list `Z80_Interrupt_CSR = {AF,BC,DE,HL,IX,IY}` is filtered down to only the registers the function body actually clobbers, then PUSHed/POPed automatically.  Lit test `llvm/test/CodeGen/Z80/interrupt.ll` confirms one-store ISRs emit `push af / ... / pop af / ei / reti`.  Requires rewriting each ISR body as proper C with inline asm clobber lists (the OUT/IN sequences); current naked-asm form gives the same end result so this is purely an ergonomics / compiler-driven-reg-tracking improvement.
