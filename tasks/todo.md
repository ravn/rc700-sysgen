...
- [ ] **Rewrite ISR routines in C** (user request 2026-04-29).  `isr_crt`, `isr_pio_kbd`, `isr_pio_par` in `cpnos-rom/isr.c` are currently `__attribute__((naked))` with hand-written PUSH/POP + body.  Convert each to a proper C function with `__attribute__((interrupt))`; clang then emits the right prologue/epilogue automatically.
  - llvm-z80 already supports the attribute (`Z80FrameLowering.cpp:386-398`, `Z80CallLowering.cpp:245-264`, `Z80RegisterInfo.cpp:104-112`).
  - Without `+shadow-regs` (our current build) the CSR list `Z80_Interrupt_CSR = {AF,BC,DE,HL,IX,IY}` is filtered to only the registers the function body actually clobbers (per `spillCalleeSavedRegisters`).
  - Lit test `llvm/test/CodeGen/Z80/interrupt.ll` confirms a one-store ISR emits exactly `push af / ... / pop af / ei / reti`.
  - The Z80 has no port-IO intrinsics (filed clang issue: address_space(2) crashes Legalizer), so the OUT/IN sequences must stay as inline asm with proper clobber lists ("a", "hl", "memory", etc.).  clang reads those clobbers and adds the matching PUSH/POP only for the regs declared.
  - End result is identical bytes to today's hand-written naked form; the win is that adding/removing a register from the body automatically flows through the save set instead of needing a manual edit.
