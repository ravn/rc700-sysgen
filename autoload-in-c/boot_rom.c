 ...
 *   - build_stamp()    Build timestamp (raw bytes in ROM)
 *   - init_fdc()       FDC Specify command (pre-boot only)
 *   - clear_screen()   Fill display with spaces (pre-boot only)
...
void build_stamp(void) __naked {
    __asm__("DEFM " BUILD_STAMP_STR);
}

/* Functions only used before prom_disable() — placed in BOOT section
 * to utilize as much as possible of the PROM bytes (which are a premium).
 * 
