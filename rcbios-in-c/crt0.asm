...
; ====================================================================
    ...
    defm " RC702"           ; +0x08: system signature (6 bytes) - this is what autoload looks for.
...
_cboot:                     ; cold boot init code
    ...
    ; _conv_tables  (384 bytes) at physical 0x100 → 0xF680 (ConvTables)
    ...
    ldir
    ; HL now at _conv_tables, copy to ConvTables
...
; TODO:  Put labels corresponding to C struct fields here for clarity.
; ====================================================================
...
; TODO: Consider putting code here to save space at boot time before relocation.
_confi_defaults_end:
...
_conv_tables:
    ; Character conversion tables (384 bytes): outcon[128] + inconv[256].
    ; Selected at build time via KBLANG define (default: Danish).
    ; Generated from CONFI.COM by extract_conv_tables.py.
    ; Copied to runtime address 0xF680 (ConvTables) by _cboot.
    IFDEF SWEDISH
    include "swedish_tables.inc"
    ELSE
    IFDEF GERMAN
    include "german_tables.inc"
    ELSE
    IFDEF UK_ASCII
    include "uk_ascii_tables.inc"
    ELSE
    IFDEF US_ASCII
    include "us_ascii_tables.inc"
    ELSE
    IFDEF FRENCH
    include "french_tables.inc"
    ELSE
    IFDEF LIBRARY
    include "library_tables.inc"
    ELSE
    include "danish_tables.inc"
    ENDIF
    ENDIF
    ENDIF
    ENDIF
...
jt_exit:    jp _bios_exit       ; 0xDA53: EXIT — delayed warm boot (dead man's switch).
...
jt_clock:   jp _bios_clock      ; 0xDA56: CLOCK — real-time clock display.
                                ;   Reads or writes 32-bit counter in rtc0:rtc2.
